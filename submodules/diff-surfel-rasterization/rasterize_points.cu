	...
	const bool debug)
{
  ...
  CHECK_INPUT(projmatrix_raw);
  CHECK_INPUT(error_img);
  CHECK_INPUT(sh);
  CHECK_INPUT(campos);
  ...
  torch::Tensor contributions = torch::full({P}, 0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		error_img.contiguous().data<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_others, radii, geomBuffer, binningBuffer, imgBuffer, contributions);
}
