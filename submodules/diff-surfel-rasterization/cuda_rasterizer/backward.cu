	...
	float* dL_dtau)
{
	...
	if (Ts_precomp != nullptr) {
		...
		glm::mat4 proj_raw = glm::mat4(
			projmatrix_raw[0], projmatrix_raw[1], projmatrix_raw[2], projmatrix_raw[3],
			projmatrix_raw[4], projmatrix_raw[5], projmatrix_raw[6], projmatrix_raw[7],
			projmatrix_raw[8], projmatrix_raw[9], projmatrix_raw[10], projmatrix_raw[11],
			projmatrix_raw[12], projmatrix_raw[13], projmatrix_raw[14], projmatrix_raw[15]
	    );
		...
		cam2pix = glm::transpose(proj_raw) * ndc2pix;
		P = world2ndc * ndc2pix;
		T = glm::transpose(M) * P;
    ...
    // propagate gradient to M in camera space 
	glm::mat4x3 dL_dM_C = cam2pix * glm::transpose(dL_dT);	
	
	// caculate the gradient to the pose, refer to Monogs : "https://github.com/muskie82/MonoGS"
	float dL_dtx = dL_dM_C[2][0];
	float dL_dty = dL_dM_C[2][1];
	float dL_dtz = dL_dM_C[2][2];

	mat33 dpC_drho = mat33::identity();
	mat33 dpC_dtheta = -mat33::skew_symmetric(p_view);
	float dL_dt[6];
	for (int i = 0; i < 3; i++) {
	...
	glm::mat3 dL_dRS_C = glm::mat3(
		glm::vec3(dL_dM_C[0]),
		glm::vec3(dL_dM_C[1]),
		glm::vec3(dL_dnormals[idx].x, dL_dnormals[idx].y, dL_dnormals[idx].z)
	);

	glm::mat3 dL_dR_C = glm::mat3(
		dL_dRS_C[0] * glm::vec3(scale.x),
		dL_dRS_C[1] * glm::vec3(scale.y),
		dL_dRS_C[2]);

	float3 R_C_0 = transformVec4x3({R[0].x, R[0].y, R[0].z}, viewmatrix);
	float3 R_C_1 = transformVec4x3({R[1].x, R[1].y, R[1].z}, viewmatrix);

	float3 R_C_c1 = make_float3(R_C_0.x, R_C_1.x, normal.x);
	float3 R_C_c2 = make_float3(R_C_0.y, R_C_1.y, normal.y);
	float3 R_C_c3 = make_float3(R_C_0.z, R_C_1.z, normal.z);

	mat33 n_RC1_x = -mat33::skew_symmetric(R_C_c1);
	mat33 n_RC2_x = -mat33::skew_symmetric(R_C_c2);
	mat33 n_RC3_x = -mat33::skew_symmetric(R_C_c3);

	float3 dL_dRC_row1 = make_float3(dL_dR_C[0][0], dL_dR_C[0][1], dL_dR_C[0][2]);
	float3 dL_dRC_row2 = make_float3(dL_dR_C[1][0], dL_dR_C[1][1], dL_dR_C[1][2]);
	float3 dL_dRC_row3 = make_float3(dL_dR_C[2][0], dL_dR_C[2][1], dL_dR_C[2][2]);

	float3 dL_dtheta = {};
	dL_dtheta.x = dot(dL_dRC_row1, n_RC1_x.cols[0]) + dot(dL_dRC_row2, n_RC2_x.cols[0]) +
				dot(dL_dRC_row3, n_RC3_x.cols[0]);
	dL_dtheta.y = dot(dL_dRC_row1, n_RC1_x.cols[1]) + dot(dL_dRC_row2, n_RC2_x.cols[1]) +
				dot(dL_dRC_row3, n_RC3_x.cols[1]);
	dL_dtheta.z = dot(dL_dRC_row1, n_RC1_x.cols[2]) + dot(dL_dRC_row2, n_RC2_x.cols[2]) +
				dot(dL_dRC_row3, n_RC3_x.cols[2]);
	...
	float* dL_dtau)
{
	...
	compute_transmat_aabb(
		...
		projmatrix, projmatrix_raw, viewmatrix, W, H, 
	...
	if (shs)
		computeColorFromSH(idx, D, M, (glm::vec3*)means3D, *campos, shs, clamped, (glm::vec3*)dL_dcolors, (glm::vec3*)dL_dmean3Ds, (glm::vec3*)dL_dshs, dL_dtau);
