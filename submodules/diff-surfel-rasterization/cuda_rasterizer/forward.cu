...
renderCUDA(
	...
	const float* __restrict__ error_img,
	...
	float * __restrict__ contributions)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			//Every gaussian's contribution to the error map
			float err_contribution = error_img[pix_id] * w;
			atomicAdd(&(contributions[collected_id[j]]), err_contribution);
