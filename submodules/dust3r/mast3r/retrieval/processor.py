...
from .model import RetrievalModel, extract_local_features
...
class Retriever(object):
    ...
    def add(self, img, uid):
        feat, _, _ = self.model.forward_local(img)
        feat = feat.flatten(0, 1)
        feat_id = uid*torch.ones_like(feat[:, 0]).to(dtype=torch.int64)
        feat = feat.cpu().numpy()
        feat_id = feat_id.cpu().numpy()

        self.asmk_dataset, self.builder = self.asmk.build_ivf_I(feat, feat_id, pre_builder=self.builder)
    
    def query(self, img, uid):
        feat, _, _ = self.model.forward_local(img)         
        feat = feat.flatten(0, 1)
        feat_id = uid*torch.ones_like(feat[:, 0]).to(dtype=torch.int64)
        feat = feat.cpu().numpy()
        ...
        metadata, query_ids, ranks, ranked_scores = self.asmk_dataset.query_ivf(feat, feat_id)

        return ranks, ranked_scores
